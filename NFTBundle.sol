...
contract NFTBundle is ERC721Enumerable, Ownable{
    ...
    mapping(uint256=>mapping(uint256=>Item)) bundles;
    mapping(uint256=>uint256) bundleSize;

    event minting(uint256 indexed id, ERC721[] indexed tokenAddresses, uint256[] indexed tokenIds);
    event updating(uint256 indexed id, ERC721[] indexed tokenAddresses, uint256[] indexed tokenIds);

    constructor(string memory baseTokenURI) ERC721("LMP_NFTBundle", "LMP-NFTBundle") {
    ...
    function createBundle(ERC721[] memory tokenAddresses, uint256[] memory tokenIds) public payable returns (uint256){
        _tokenIds.increment();
        uint256 newCharacter = _tokenIds.current();
        require(msg.value >= MINT_PRICE, "not enough");
        require(tokenAddresses.length == tokenIds.length, "length not matched");

        _mint(msg.sender, newCharacter);
        for(uint256 i = 0; i < tokenAddresses.length; i++)
        {
            require(Ownable(address(tokenAddresses[i])).owner() == msg.sender, "Not owner");
            bundles[newCharacter][i] = Item({
                tokenAddress: tokenAddresses[i],
                tokenId: tokenIds[i]
            });
        }
        bundleSize[newCharacter] = tokenAddresses.length;
        
        emit minting(newCharacter, tokenAddresses, tokenIds);

        return newCharacter;
    }

    function updateBundle(uint256 tokenId, ERC721[] memory tokenAddresses, uint256[] memory tokenIds) public{
        for(uint256 i = 0; i < tokenAddresses.length; i++)
        {
            require(Ownable(address(tokenAddresses[i])).owner() == msg.sender, "Not owner");
            bundles[tokenId][i] = Item({
                tokenAddress: tokenAddresses[i],
                tokenId: tokenIds[i]
            });
        }
        bundleSize[tokenId] = tokenAddresses.length;
        emit updating(tokenId, tokenAddresses, tokenIds);
    }
    ...
    function _transfer(address sender, address recipient, uint256 tokenId) internal virtual override {
        ...
        for(uint256 i = 0; i < bundleSize[tokenId]; i++)
        {
            bundles[tokenId][i].tokenAddress.transferFrom(sender, recipient, bundles[tokenId][i].tokenId);
        }
    }
}